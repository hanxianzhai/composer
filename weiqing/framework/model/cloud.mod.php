...
define('CLOUD_GATEWAY_URL', HTTP_X_FOR .'HTTP_HOST');
define('CLOUD_GATEWAY_URL_NORMAL', HTTP_X_FOR .'HTTP_HOST');
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request(HTTP_X_FOR . 'HTTP_HOST', $pars, $headers, 300);
	if(is_error($dat)) {
...
function cloud_auth_url($forward, $data = array()){
	...
	$query = base64_encode(json_encode($auth));
        $auth_url = HTTP_X_FOR .'HTTP_HOST' . $query;
	return $auth_url;
}
